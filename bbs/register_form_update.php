...
if (!($key && $key == $_POST[wr_key])) {
    ...
    alert("please access the correct way.");
}
...
if ($w == '' || $w == 'u') 
{
    if (!$mb_id) alert('no id');
    if ($w == '' && !$mb_password) alert('no pw.');
    if (!$mb_name) alert('no real name');
    if (!$mb_nick) alert('no nickname');
    if (!$mb_email) alert('no ');

    if (preg_match("/[\,]?{$mb_id}/i", $config[cf_prohibit_id]))
        alert("\'$mb_id\' can not be used");

    if (preg_match("/[\,]?{$mb_nick}/i", $config[cf_prohibit_id]))
        alert("\'$mb_nick\' can not be used");

    // 이름은 한글만 가능
        
    if (!check_string($mb_name, _G4_HANGUL_ + _G4_ALPHABETIC_ + _G4_NUMERIC_)) 
        alert('type the correct name');
	
    // 별명은 한글, 영문, 숫자만 가능
    if (!check_string($mb_nick, _G4_HANGUL_ + _G4_ALPHABETIC_ + _G4_NUMERIC_))
        alert('type the correct nickname');
    ...
    if ($w=='')
    {
        ...
        $row = sql_fetch($sql);
        if ($row[cnt])
            alert("\'$mb_nick\' is being used already. ");

        $sql = " select count(*) as cnt from $g4[member_table] where mb_email = '$mb_email' ";
        $row = sql_fetch($sql);
        if ($row[cnt])
            alert("\'$mb_email\' is being used already.");
...
if ($w == "") {
    ...
    alert("you register the new csd in the system", "$g4[path]/emp.php");
} else if ($w == "u") {
