    	...
    	<td colspan=<?=$colspan?> align=center>
    		<div class="form_left">
    			<table style="border: solid #CCC 1px;" >
    				<colgroup width="" ></colgroup>
					<colgroup width="" ></colgroup>
					<thead align="center" style="background-color : eee5e5">
						<tr>
							<td>category</td>
							<td>data</td>
						</tr>
					...
					<tbody align="center">
						<tr>
							<td>member id</td>
							<td><?= $member['mb_5'] == "on" ?  $row['mb_id'] : "n/a"?></td>
						</tr>
						<tr>
							<td>member phone #</td>
							<td><?= $member['mb_6'] == "on" ? $row['mb_number'] : "n/a" ?></td>
						</tr>
						<tr>
							<td>email</td>
							<td><?= $member['mb_7'] == "on" ? $row['mb_email'] : "n/a" ?></td>
						</tr>
						<tr>
							<td>event</td>
							<td><?= $member['mb_8'] == "on" ? $row['event'] : "n/a" ?></td>
						</tr>
						<tr>
							<td>country</td>
							<td><?= $member['mb_9'] == "on" ? $row['country'] : "n/a"?></td>
						</tr>
					</tbody>
    			</table>
    		</div>
    		
    		<form name=fboardlist<?= $row['seq']?>  method=post>
    		<input type='hidden' name='seq' value='<?= $row['seq']?>'>
	    		<div class="form_middle">
					<table style="border: solid #CCC 1px;" >
						...
						<tbody align="center">
							<tr>
								<td><textarea class="ed" name="call_content" rows="10" style="width:99%;"></textarea></td>
							</tr>
							<tr>
								<td><button onclick="multiple_form( this.form )">submit</button></td>
							</tr>
						</tbody>
    				</table>
	    		</div>
...
function show_layer(no)
{
	...
	var myArticle = $(".row"+no);
	 ...
	 if( $(".CountOn").length >= 1 ){
	 	$(".CountOn").fadeOut("slow");
	 	myArticle.stop().addClass('CountOn').fadeIn('slow');
	 	return;
	 }
...
function multiple_form(f)
{

	if( f.call_content.value.length < 5 ){
		alert('type the content more than 5');
		return;
	}
    
    if(!confirm("Do you want to proceed now?")) {
    	return;
    	
    }
	
	f.action = "./csd_update.php";
