...
function get_paging($write_pages, $cur_page, $total_page, $url, $add="")
{
	...
	$str = "";
    if ($cur_page > 1) {
        $str .= "<a href='" . $url . "1{$add}' class='pre_all'>{$lang[351]}</a>";
        $str .= "<a href='" . $url . ($cur_page-1) . "' class='pre'>{$lang[352]}</a>";
    }
 else
 {
     $str .= "<span class='pre_all cur'>{$lang[351]}</span>";
     $str .= "<span class='pre cur'>{$lang[352]}</span>";
 }
    ...
    if ($total_page > 1) {
        for ($k=$start_page;$k<=$end_page;$k++) {
            if ($cur_page != $k)
                ...
                $str .= "<b class='cur' style='color : blue'><i>$k</i></b>";
        }
    }
 ...
 if($total_page < $page_count) $page_count=$total_page;
    ...
    if ($cur_page < $total_page) {
        $str .= "<a href='$url" . ($cur_page+1) . "' class='next'>{$lang[353]}</a>";
        $str .= "<a href='$url$total_page{$add}' class='next_all'>{$lang[354]}</a>";
    }
 else
 {
     $str .= "<span class='next cur'>{$lang[353]}</span>";
     $str .= "<span class='next_all cur'>{$lang[354]}</span>";
 }
    $str .= "";
    return $str;
}

function get_paging_jquery($write_pages, $cur_page, $total_page, $add="")
{
    global $lang;
	$str = "<div class='pagination pagination-centered'>
                  <ul>";
    if ($cur_page > 1) {
        $str .= "<li><a rel='1{$add}' href='#' class='pre_all'>{$lang[351]}</a></li>";
        $str .= "<li><a rel='".($cur_page-1)."' href='#' class='pre'>{$lang[352]}</a></li>";
    }
 else
 {
     $str .= "<li><a class='pre_all cur'>{$lang[351]}</a></li>";
     $str .= "<li><a class='pre cur'>{$lang[352]}</a></li>";
 }
    $start_page = ( ( (int)( ($cur_page - 1 ) / $write_pages ) ) * $write_pages ) + 1;
    $end_page = $start_page + $write_pages - 1;
    if ($end_page >= $total_page) $end_page = $total_page;
 ...
 $start_paging=$start_page-1;
    ...
    if ($total_page > 1) {
        for ($k=$start_page;$k<=$end_page;$k++) {
            if ($cur_page != $k)
                $str .= "<li><a rel='$k{$add}' href='#'  class='numBox;'>$k</a></li>";
            else
                $str .= "<li><a rel='$k'><b class='cur' style='color : blue'><i>$k</i></b></a></li>";
        }
    }
 $end_paging=$end_page +1;
 $page_count=$end_page+$write_pages;
 if($total_page < $page_count) $page_count=$total_page;
    //if ($total_page > $end_page) $str .= "<a href='" . $url . ($end_page+1) . "{$add}'>$end_paging ~ $page_count</a>";
    if ($cur_page < $total_page) {
        $str .= "<li><a rel='".($cur_page+1)."' href='#' class='next'>{$lang[353]}</a></li>";
        $str .= "<li><a rel='$total_page{$add}' rel='$url$total_page{$add}' href='#' class='next_all'>{$lang[354]}</a></li>";
    }
 else
 {
     $str .= "<li><a rel='$total_page{$add}' class='next cur'>{$lang[353]}</a>";
     $str .= "<li><a rel='$total_page{$add}' class='next_all cur'>{$lang[354]}</a>";
 }
    $str .= "</ul></div>";
    return $str;
}
...
function get_yn_select($name, $selected='1', $event='')
{
    $str = "<select name='$name' $event>";
    if ($selected) {
        $str .= "<option value='1' selected>{$lang[540]}</option>";
        $str .= "<option value='0'>{$lang[541]}</option>";
    } else {
        $str .= "<option value='1'>{$lang[540]}</option>";
        $str .= "<option value='0' selected>{$lang[541]}</option>";
    }
    $str .= "</select>";
    return $str;
}
...
{
    $arr_yoil = array ("{$lang[543]}", "{$lang[544]}", "{$lang[545]}", "{$lang[546]}", "{$lang[547]}", "{$lang[548]}", "{$lang[549]}");
...
function bad_tag_convert($code)
{
    ...
    if ($is_admin && $member[mb_id] != $view[mb_id]) {
        $code = preg_replace_callback("#(\<(embed|object)[^\>]*)\>(\<\/(embed|object)\>)?#i",
                    create_function('$matches', 'return "<div class=\"embedx\">{$lang[554]}</div>";'),
                    $code);
    }
