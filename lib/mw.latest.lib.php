...
function mw_latest($skin_dir="", $bo_table, $rows=10, $subject_len=40, $is_img=0, $minute=0)
{
	...
	$sql.= "order by bf_datetime desc limit $is_img";
	$qry = sql_query($sql);
	for ($i=0; $row=sql_fetch_array($qry); $i++) {
	    ...
	    if ($file[$i]) {
		$row2 = sql_fetch("select wr_subject from $g4[write_prefix]$row[bo_table] where wr_id = '$row[wr_id]'");
		$file[$i][subject] = $row2[wr_subject];
	    }
	}
