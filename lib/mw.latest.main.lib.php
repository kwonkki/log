...
function mw_latest_main($skin_dir="", $bo_tables, $rows=10, $subject_len=40, $minute=0)
{
	...
	for ($i=0; $row = sql_fetch_array($qry); $i++) {
	    ...
	    if ($file[$i]) {
		$row = sql_fetch("select wr_subject from $g4[write_prefix]$row[bo_table] where wr_id = '$row[wr_id]'");
		$file[$i][subject] = $row[wr_subject];
	    }
	...
	for ($i=0; $row = sql_fetch_array($qry); $i++) {
	    ...
	    $sql = "select wr_subject,ca_name from $tmp_write_table where wr_id = '$row[wr_id]'";
	    $row2 = sql_fetch($sql);
