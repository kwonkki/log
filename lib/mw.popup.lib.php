...
function mw_popup() {
    ...
    $sql = "select * from $mw[popup_table] where pp_use = '1' and '$g4[time_ymd]' between pp_start and pp_end order by pp_id desc";
    $qry = sql_query($sql);
	...
	if ($row[pp_type] == "1") { // 레이어 팝업
	    ...
	    $ret.= "#$popup_name { border:1px solid #ddd; background-color:#fff; text-align:left; padding:0; }\n";
	    $ret.= "</style>\n";
