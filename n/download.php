...
if ($member[mb_level] < $board[bo_download_level]) {
    ...
    if ($member[mb_id])
        ...
        alert($alert_msg . "\\n\\n{$lang[287]}", "./login.php?wr_id=$wr_id&$qstr&url=".urlencode("$g4[bbs_path]/board.php?bo_table=$bo_table&wr_id=$wr_id"));
}
...
if (!get_session($ss_name))
{
    ...
    else if ($board[bo_download_level] > 1) // 회원이상 다운로드가 가능하다면
    {
        ...
        // 게시물당 한번만 차감하도록 수정
        insert_point($member[mb_id], $board[bo_download_point], "$board[bo_subject] $wr_id {$lang[325]}", $bo_table, $wr_id, "{$lang[89]}");
    }
...
$g4[title] = "$group[gr_subject] > $board[bo_subject] > " . conv_subject($write[wr_subject], 255) . " > {$lang[89]}";
