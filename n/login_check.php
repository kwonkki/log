...
if ($mb[mb_intercept_date] && $mb[mb_intercept_date] <= date("Ymd", $g4[server_time])) {
    $date = preg_replace("/([0-9]{4})([0-9]{2})([0-9]{2})/", "\\1{$lang[14]} \\2{$lang[13]} \\3{$lang[12]}", $mb[mb_intercept_date]); 
    alert("{$lang[367]}\\n\\n{$lang[368]} : $date");
    goto_url($g4[path]);
}
...
if ($mb[mb_leave_date] && $mb[mb_leave_date] <= date("Ymd", $g4[server_time])) {
    $date = preg_replace("/([0-9]{4})([0-9]{2})([0-9]{2})/", "\\1{$lang[14]} \\2{$lang[13]} \\3{$lang[12]}", $mb[mb_leave_date]); 
    alert("{$lang[369]}\\n\\n{$lang[370]} : $date");
    goto_url($g4[path]);
}

if ($config[cf_use_email_certify] && !preg_match("/[1-9]/", $mb[mb_email_certify]))
    alert("{$lang[371]}\\n\\n{$lang[372]} $mb[mb_email]");
