...
if (!$po[po_id]) 
    alert_close("po_id {$lang[355]}");

if ($member[mb_level] < $po[po_level]) 
    alert_close("{$lang[422]} $po[po_level] {$lang[423]}");
...
if (get_cookie("ck_po_id") != $po[po_id]) 
{
    ...
    if (!$search_mb_id)
        insert_point($member[mb_id], $po[po_point], $po[po_id] . ". " . cut_str($po[po_subject],20) . " {$lang[424]} ", "@poll", $po[po_id], "{$lang[425]}");
}
