...
{
    if (isset($wr_id))
        alert("{$lang[489]}", "$g4[bbs_path]/board.php?bo_table=$bo_table");

    if ($member[mb_level] < $board[bo_write_level]) {
        if ($member[mb_id])
            alert("{$lang[490]}");
        else
            alert("{$lang[490]}", "./login.php?$qstr&url=".urlencode("$_SERVER[PHP_SELF]?bo_table=$bo_table"));
    }
...
else if ($w == "u")
{
    ...
    if($member['mb_id'] && $write['mb_id'] == $member['mb_id'])
        ...
        if ($member[mb_id])
            ...
            alert("{$lang[493]}", "./login.php?$qstr&url=".urlencode("$_SERVER[PHP_SELF]?bo_table=$bo_table"));
    }
...
else if ($w == "r")
{
    if ($member[mb_level] < $board[bo_reply_level]) {
        if ($member[mb_id])
            ...
            alert("{$lang[497]}", "./login.php?$qstr&url=".urlencode("$_SERVER[PHP_SELF]?bo_table=$bo_table"));
    }
...
{
    if (!$member[mb_id])
        alert("{$lang[504]}", "login.php?$qstr&url=".urlencode("$_SERVER[PHP_SELF]?bo_table=$bo_table"));
...
if ($w == "u")
{
    for ($i=0; $i<$file[count]; $i++)
    {
        ...
        if ($row[bf_file])
        {
            $file_script .= "add_file(\"<input type='checkbox' name='bf_file_del[$i]' value='1'><a href='{$file[$i][href]}'>{$file[$i][source]}({$file[$i][size]})</a> {$lang[507]}";
            if ($is_file_content)
                ...
                $file_script .= "<br><input type='text' class=ed size=50 name='bf_content[$i]' value='".addslashes(get_text($row[bf_content]))."' title='{$lang[508]}'>";
            $file_script .= "\");\n";
        }
        else
