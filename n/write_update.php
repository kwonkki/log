...
for ($i=0; $i<count($_FILES[bf_file][name]); $i++) 
{
    ...
    if ($filename)
    {
        ...
        {
            $file_upload_msg .= "\'{$filename}\' {$lang[532]}\\n";
            continue;
        }
        else if ($_FILES[bf_file][error][$i] != 0)
        {
            $file_upload_msg .= "\'{$filename}\' {$lang[533]}\\n";
            continue;
        }
    }
    ...
    if (is_uploaded_file($tmp_file)) 
    {
        ...
        {
            $file_upload_msg .= "\'{$filename}\' {$lang[534]}\\n";
            continue;
        }
...
if ($w == "" || $w == "r") 
{
    ...
    if ($w == '') 
    {
        ...
        insert_point($member[mb_id], $board[bo_write_point], "$board[bo_subject] $wr_id {$lang[76]}", $bo_table, $wr_id, '{$lang[76]}');
    }
    else 
    {
        // 답변은 코멘트 포인트를 부여함
        // 답변 포인트가 많은 경우 코멘트 대신 답변을 하는 경우가 많음
        insert_point($member[mb_id], $board[bo_comment_point], "$board[bo_subject] $wr_id {$lang[503]}", $bo_table, $wr_id, '{$lang[503]}');
    }
} 
else if ($w == "u") 
{
    if ($is_admin == "super") // 최고관리자 통과
        ...
        if ($write[mb_id])
            alert("{$lang[539]}", "./login.php?url=".urlencode("./board.php?bo_table=$bo_table&wr_id=$wr_id"));
    }
...
if (!($w == "u" || $w == "cu") && $config[cf_email_use] && $board[bo_use_email]) 
{
    ...
    $warr = array( ""=>"{$lang[518]}", "u"=>"{$lang[81]}", "r"=>"{$lang[83]}", "c"=>"{$lang[255]}", "cu"=>"{$lang[519]}" );
    $str = $warr[$w];

    $subject = "'{$board[bo_subject]}' {$lang[520]} {$str}{$lang[521]}";
