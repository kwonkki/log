...
if ($mw_basic[cf_contents_shop] == "1") $colspan++;
...
<tr class=mw_basic_list_title>
    ...
    <? if ($mw_basic[cf_contents_shop] == "1") { ?> <td width=80><?=$mw_cash[cf_cash_name]?></td> <?}?>
...
?>
    <td width="<?=$td_width?>%" class="mw_basic_list_gall <?=$class?>">
        <? if ($is_checkbox) { ?>
            <div style="text-align:left; width:<?=$set_width+18?>px; margin:0 auto 0 auto;'"><input type="checkbox" name="chk_wr_id[]" value="<?=$list[$i][wr_id]?>"></div> <? } ?>
        <div><a href="<?=$list[$i][href]?>"><img src="<?=$thumb_file?>" <?=$thumb_width?> <?=$thumb_height?> align=absmiddle></a></div>
        <div class="mw_basic_list_subject_gall" <? if (!$mw_basic[cf_thumb_keep]) echo "style='width:".($set_width+10)."px; text-align:left;'"; ?>>
            <? if ($is_category && $list[$i][ca_name]) { ?> <div style="margin:0 0 5px 0;"><a href="<?=$list[$i][ca_name_href]?>" class=mw_basic_list_category>[<?=$list[$i][ca_name]?>]</a></div> <? } ?>
