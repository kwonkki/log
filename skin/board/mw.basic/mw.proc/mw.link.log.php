...
$sql_common = " from $mw[link_log_table] ";
$sql_order = " order by ll_id desc ";
...
$write_pages = get_paging($config[cf_write_pages], $page, $total_page, "./mw.link.log.php?bo_table={$bo_table}&wr_id={$wr_id}{$qstr}&page=");
...
<tr style="text-align:center; height:25px;">
    ...
    <td> <?=get_sideview($list[$i][mb_id], $list[$i][ll_name])?> </td>
    <td> <?=$list[$i][ll_ip]?> </td>
    <td> <?=$list[$i][ll_datetime]?> </td>
</tr>
<tr><td colspan=<?=$colspan?> bgcolor="#f8f8f8"></td></tr>
<? } ?>
<? if ($i == 0) { ?>
<tr><td colspan=<?=$colspan?> height=100 align=center> 링크 기록이 없습니다. </td></tr>
<? } ?>
</table>
