...
if ($mw_basic[cf_comment_best]) {
    ...
    $sql.= " order by wr_good desc, wr_datetime asc limit 1 ";
    $row = sql_fetch($sql);
    if ($row) {
    ...
    if ($mw_basic[cf_attribute] == "anonymous") $row[name] = "익명"; 
    if ($row[wr_anonymous]) $row[name] = "익명"; 
...
<td width="100%" valign="top">
    ...
    <tr>
        ...
        <td>
            ...
            <? if ($write[wr_qna_id] == $row[wr_id] ) { ?> <img src="<?=$board_skin_path?>/img/icon_choose.png" align="absmiddle"> <? } ?>
            <span class=mw_basic_comment_name><?=$row[name]?></span>
            <? if ($is_ip_view) { ?> <span class=mw_basic_comment_ip>(<?=$row[ip]?>)</span> <?}?>
            <? if ($is_admin) { ?>
...
for ($i=$from_record; $i<$to_record; $i++) {
    ...
    if ($list[$i][wr_anonymous]) $list[$i][name] = "익명"; 
    ...
    if ($is_member) {
        $list[$i][content] = str_replace("{닉네임}", $member[mb_nick], $list[$i][content]);
    } else {
        $list[$i][content] = str_replace("{닉네임}", "회원", $list[$i][content]);
    }
?>
...
<tr>
    ...
    <td width="100%" valign="top">
        ...
        <tr>
            <!-- 이름, 아이피 -->
            <td>
                <? if ($write[wr_qna_id] == $list[$i][wr_id] ) { ?> <img src="<?=$board_skin_path?>/img/icon_choose.png" align="absmiddle"> <? } ?>
        ...
        <tr>
            <td valign="top">
                <?  if ($list[$i][wr_id] == $is_comment_best) { ?>
                <div id="info_best_reply">
                    베플로 선택된 게시물입니다.
                    <input type="button" value="원문확인▼" id="btn_best_reply_view">
                </div>
                <style type="text/css">
                #info_best_reply { margin:0 0 20px 0; color:#F56C07; }
                #view_<?=$list[$i][wr_id]?> { display:none; } 
                #btn_best_reply_view { background-color:#fff; color:#444; cursor:pointer; font-size:12px; border:0; }
                </style>
                <script type="text/javascript">
                $("#btn_best_reply_view").click(function () {
...
<tr>
    <td>
        ...
        <textarea id="wr_content" name="wr_content" rows="6" itemname="내용" required
            ...
            <? if (!$is_comment_editor && ($comment_min || $comment_max)) { ?>onkeyup="check_byte('wr_content', 'char_count');"<?}?> class=mw_basic_textarea style='width:100%; word-break:break-all;' ><?=$mw_basic[cf_comment_default]?></textarea>
    ...
    <td width=60 align=center id="btn_submit">
        <div><input type="image" src="<?=$board_skin_path?>/img/btn_comment_ok.gif" border=0 accesskey='s' <?=$write_error?>></div>
        ...
        <div style="margin-top:5px;"><input type="image" src="<?=$board_skin_path?>/img/btn_comment_n_good_ok.gif" border=0 onclick="mw_good_act('good')"></div>
        <? } ?>
    </td>
</tr>
</table>
...
function mw_qna_choose(wr_id) {
    if (wr_id) {
        if (!confirm("이 답변을 채택하시겠습니까?")) return false;
    } else {
        if (!confirm("이 질문을 보류하시겠습니까?")) return false;
    }
